...
def AddPrefixToFields(input_file, output_file, prefix, field = [0]):
    ...
    for line in list:
        if len(line) > 0 and line[0] != ';':
            split1 = line.split()
            for j in field:
                split1[j] = GetNewId(split1[j], prefix)
...
def main():
    ...
    for line in wav_scp_file:
        ...
        new_wav = AugmentWav(utt, wav, dur, fg_snrs, bg_snrs, fg_noise_utts,
            ...
            num_bg_noises)
        new_utt = GetNewId(utt, args.utt_prefix)
    ...
    if os.path.isfile(input_dir + "/utt2uniq"):
        AddPrefixToFields(input_dir + "/utt2uniq", output_dir + "/utt2uniq", args.utt_prefix, field =[0])
    else:
        # Create the utt2uniq file
        CreateCorruptedUtt2uniq(input_dir, output_dir, args.utt_prefix)

    if os.path.isfile(input_dir + "/text"):
        AddPrefixToFields(input_dir + "/text", output_dir + "/text", args.utt_prefix, field =[0])
    if os.path.isfile(input_dir + "/segments"):
        AddPrefixToFields(input_dir + "/segments", output_dir + "/segments", args.utt_prefix, field = [0,1])
    if os.path.isfile(input_dir + "/reco2file_and_channel"):
        AddPrefixToFields(input_dir + "/reco2file_and_channel", output_dir + "/reco2file_and_channel", args.utt_prefix, field = [0,1])

    data_lib.RunKaldiCommand("utils/fix_data_dir.sh {output_dir}"
                    .format(output_dir = output_dir))
