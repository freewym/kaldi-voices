...
local/nnet3/run_ivector_common.sh \
  --stage $stage --nj $nj \
  --train-set $train_set --gmm $gmm --combined-train-set $combined_train_set --aug-prefix $aug_prefix \
  --nnet3-affix "$nnet3_affix" || exit 1;


gmm_dir=exp/${gmm}
ali_dir=exp/${gmm}_ali_${train_set}_sp
combined_ali_dir=exp/${gmm}_ali_${combined_train_set}_sp
tree_dir=exp/chain${nnet3_affix}/tree_sp${tree_affix:+_$tree_affix}
lang=data/lang_chain
lat_dir=exp/chain${nnet3_affix}/${gmm}_${train_set}_sp_lats
combined_lat_dir=exp/chain${nnet3_affix}/${gmm}_${combined_train_set}_sp_lats
dir=exp/chain${nnet3_affix}/tdnn${affix}_sp
train_data_dir=data/${train_set}_sp_hires
combined_train_data_dir=data/${combined_train_set}_sp_hires
train_ivector_dir=exp/nnet3${nnet3_affix}/ivectors_${train_set}_sp_hires
combined_train_ivector_dir=exp/nnet3${nnet3_affix}/ivectors_${combined_train_set}_sp_hires
...
for f in $train_data_dir/feats.scp $combined_train_data_dir/feats.scp \
  $train_ivector_dir/ivector_online.scp $combined_train_ivector_dir/ivector_online.scp \
    $lores_train_data_dir/feats.scp $combined_lores_train_data_dir/feats.scp \
    $gmm_dir/final.mdl $ali_dir/ali.1.gz $combined_ali_dir/ali.1.gz; do
...
if [ $stage -le 13 ]; then
  local/copy_lat_dir.sh --nj 75 --utt-prefixes $(echo $aug_prefix | sed 's/reverb/reverb1/g') \
